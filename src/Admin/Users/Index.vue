				...
				.box.box-default
					...
					table.table.table-condensed.table-hover.table-responsive
						thead
							tr
								th
								th.hidden-sm.hidden-xs First Name
								th.hidden-sm.hidden-xs Last Name
								th Username
								th Email
								th.hidden-xs Role
								th Enabled
						...
						tbody
							tr(v-for="user in users | filterBy search in 'fname' 'lname' 'username' 'email'")
								...
								td.hidden-sm.hidden-xs: a(v-link="{path:'/admin/users/view/' + user._id}") {{user.fname}}
								td.hidden-sm.hidden-xs {{user.lname}}
								td: a(v-link="{path:'/admin/users/view/' + user._id}") {{user.username}}
								td {{user.email}}
